...
void CameraControl::Load_default_setting(){
    ...
    while(!in.atEnd()){
        ...
        if(temp.contains("[Pixelfly Camera Image Number]")){
            QString a = in.readLine();
            ui->Image_Nr->setCurrentIndex(a.toInt());
        }
        else if(temp.contains("[Pixelfly Camera Trigger Mode]")){
            QString a = in.readLine();
            if(a.contains("TTL")){
        ...
        else if(temp.contains("[Pixelfly Camera Save Path]")){
            QString a = in.readLine();
            ui->Image_Path->setText(a);
        }
        else if(temp.contains("[Pixelfly Camera File Name]")){
            QString a = in.readLine();
            ui->Image_Name->setText(a);
        }
        else if(temp.contains("[Pixelfly Camera Exposure Time]")){
            QString a = in.readLine();
...
void CameraControl::on_Preview_clicked()
{
...
void CameraControl::update_preview(){
    emit start_acquire(1000/Camera_Frame_Rate);
}

void CameraControl::preview_close(){
    preview->deleteLater();
    delete preview;
    cam->Stop_Recording();
    cam->Free_Buffer();
...
void CameraControl::on_Connect_clicked()
{
...
void CameraControl::on_Reset_clicked()
{
    cam->Close_Camera();
    cam->disconnect();
    isArm = false;
...
void CameraControl::on_Arm_clicked()
{
    if(isArm){
...
void CameraControl::on_Path_clicked()
{
...
void CameraControl::on_Exposure_returnPressed()
{
    bool ok = false;
...
void CameraControl::Reload(QString msg){
...
void CameraControl::acquire_next_image(){
    if(ui->Trigger_mode->currentIndex()==1){
...
void CameraControl::image_time_out(){
...
void CameraControl::receive_image(QList<int> img_dat){
    if(ui->Arm->text()!="Release")return;
...
void CameraControl::save_images(int i){
...
void CameraControl::on_Trigger_clicked()
{
    acquire_next_image();
}
